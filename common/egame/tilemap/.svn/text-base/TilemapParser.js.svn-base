egame.define("TilemapParser",["ImageCollection"],function(ImageCollection) {

    /**
     * egame.TilemapParser parses data objects from egame.Loader that need more preparation before they can be inserted into a Tilemap.
     *
     * @class egame.TilemapParser
     * @static
     */
    egame.TilemapParser = {
         ...
         * a egame.Tile instance with an index of -1 (false, the default). Depending on your game type
         ...
         *
         * @method egame.TilemapParser.parse
        ...
        parse: function(game, key, tileWidth, tileHeight, width, height) {
            ...
            var json = egame.Caches[key].data;

            if (json) {
                return this.parseTiledJSON(json);
                // if (map.format === egame.Tilemap.CSV) {
                //     return this.parseCSV(key, map.data, tileWidth, tileHeight);
                // } else if (!map.format || map.format === egame.Tilemap.TILED_JSON) {
                //     return this.parseTiledJSON(map.data);
                // }
            } else {
                console.warn('egame.TilemapParser.parse - No map data found for key ' + key);
            }
         ...
         *
         * @method egame.TilemapParser.parseCSV
        ...
        parseCSV: function(key, data, tileWidth, tileHeight) {
            ...
            for (var y = 0; y < rows.length; y++) {
                ...
                for (var x = 0; x < column.length; x++) {
                    output[y][x] = new egame.Tile(map.layers[0], parseInt(column[x], 10), x, y, tileWidth, tileHeight);
                }
         ...
         *
         * @method egame.TilemapParser.getEmptyData
         * @return {object} Generated map data.
         */
         ...
         * @method egame.TilemapParser.parseJSON
        ...
        parseTiledJSON: function(json) {
            ...
            if (json.orientation !== 'orthogonal') {
                console.warn('TilemapParser.parseTiledJSON - Only orthogonal map types are supported in this version of egame');
                return null;
            }
            ...
            map.format = egame.Tilemap.TILED_JSON;
            ...
            for (var i = 0; i < json.layers.length; i++) {
                ...
                for (var t = 0, len = curl.data.length; t < len; t++) {
                    ...
                    if (gid > 0) {
                        row.push(new egame.Tile(layer, gid, x, output.length, json.tilewidth, json.tileheight));
                        row[row.length - 1].rotation = rotation;
                        row[row.length - 1].flipped = flipped;
                    } else {
                        if (egame.TilemapParser.INSERT_NULL) {
                            row.push(null);
                        } else {
                            row.push(new egame.Tile(layer, -1, x, output.length, json.tilewidth, json.tileheight));
                        }
                    }
            ...
            for (var i = 0; i < json.tilesets.length; i++) {
                ...
                if (set.image) {
                    var newSet = new egame.Tileset(set.name, set.firstgid, set.tilewidth, set.tileheight, set.margin, set.spacing, set.properties);
                    ...
                    var newCollection = new egame.ImageCollection(set.name, set.firstgid, set.tilewidth, set.tileheight, set.margin, set.spacing, set.properties);
                ...
                for (var j = 0; j < layer.data.length; j++) {
                    ...
                    for (var k = 0; k < row.length; k++) {
                        ...
                        if (set.tileProperties && set.tileProperties[tile.index - set.firstgid]) {
                            tile.properties = egame.util.extend(set.tileProperties[tile.index - set.firstgid], tile.properties);
                        }
                    }
                }
            }
